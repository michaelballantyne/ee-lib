...
This library provides a higher-level API to Racket's syntax system, designed for implementing macro expanders for DSLs. The paper @hyperlink["https://dl.acm.org/doi/pdf/10.1145/3428297"]{"Macros for Domain-Specific Languages"} serves as the guide-level explanation of the library and associated programming patterns. This page provides reference documentation.
...
Intuitively, a scope is a region of program text in which certain bindings are available, determined by scoping forms such as @racket[let] or @racket[block]. In the context of macro expansion, syntax may be moved in and out of scopes during the process of expansion, and some scopes aren't immediately evident in the source program text. The Racket expander uses several kinds of @tech/reference{scope} values to represent regions of partially expanded programs to implement macro hygiene. This documentation refers to these scope values as @deftech{scope tags}, and reserves the word "scope" for the intuitive notion.

@defform[(with-scope id body ...)]{
Introduces the elements needed to implement a scope: an @tech/reference{outside-edge scope} tag, an @tech/reference{inside-edge scope} tag, and a new @tech{binding context segment}.

The two scope tags are encapsulated in a @deftech{scope tagger} value accessible via @racket[id]. The new @tech{binding context segment} is added to the @tech{library local binding context} for the dynamic extent of the evaluation of the @racket[body] forms.
}
...
Annotates the @racket[stx] with the @tech{scope tags} represented by the @racket[tagger].
}
...
Flips the current @tech/reference{macro-introduction scope} and removes any @tech/reference{use-site scope} tags created for the current expansion context.
...
This library implicitly maintains a @deftech{library local binding context} with entries that map bindings to values, similar to the core expander's @tech/reference{local binding context}. See @racket[current-def-ctx] for a way to access the library local binding context as a first-class definition context that can be used with the core expander's API.

The library local binding context consists of nested @deftech{binding context segments} corresponding to the nested dynamic extents of @racket[with-scope] uses. The @racket[bind!] operation adds new entries to the innermost segment. The binding context may be @deftech{sealed}, preventing further bindings within the context until a new segment is added. All operations that use syntax to create or lookup bindings in the binding context first annotate the syntax with the @tech/reference{inside-edge scope} tag for the scope corresponding to the innermost binding context segment.
...
This operation is legal only in a @tech{library local binding context} with an @tech[#:key "sealed"]{unsealed} segment.
...
Looks for a binding and corresponding entry in the @tech{library local binding context} for @racket[id]. If the binding exists, has an value in the @tech{library local binding context}, and the value satisfies @racket[predicate], the value is returned and a @tech{disappeared use} is recorded for @racket[id]. Otherwise returns @racket[#f].
}


@subsection{Hygiene for expander definitions}

As an expander traverses syntax, it needs to enter new @seclink["expand-context-model"
         #:doc '(lib "scribblings/reference/reference.scrbl")]{expansion contexts} and adjust the @tech{library local binding context}. And hygiene should treat syntax constructed introduced by templates in the expander similarly to syntax introduced by a macro. The following forms define expand functions with these behaviors.

@defform[(define/hygienic (id arg ...) ctx-type body ...)
         #:grammar [(arg id)
                    (ctx-type #:expression #:definition)]]{
Defines a function @racket[id] with positional arguments @racket[arg ...] and body @racket[body]. Invocations of the function enter a new @seclink["expand-context-model"
         #:doc '(lib "scribblings/reference/reference.scrbl")]{expansion context} of the type specified the @racket[ctx-type]: an expression context, or an internal-definition context. Entering an expression context @tech[#:key "sealed"]{seals} the @tech{library local binding context}; entering an internal-definition context does not.

Invocations of the function are hygienic in the same way macro applications are hygienic: for syntax-valued arguments and returns, arguments are tagged by a fresh @tech/reference{use-site scope} tag, and syntax returned from the function that was not part of one of the arguments is tagged with a fresh @tech/reference{macro-introduction scope} tag.

The expansion context type determines the treatment of @tech/reference{use-site scope} tags at uses of @racket[bind], @racket[syntax-local-identfier-as-binding], and @racket[syntax-local-introduce-splice] within. During expansion in an internal-definition context, the expansion context tracks a set of use-site scopes created during expansion of the context. The operations just mentioned remove use-site scopes present in that set. Entering an expression context resets the set to empty.
}
...
Expanders also need to apply macro hygiene when invoking macros, via @racket[apply-as-transformer]. Macros should expand to forms such as @racket[define] rather than directly extending the binding context with @racket[bind!], so @racket[apply-as-transformer] @tech[#:key "sealed"]{seals} the binding context.

@defproc[(apply-as-transformer [proc procedure?] [ctx-type (or/c 'expression 'definition)]
                               [arg any/c] ...)
          any]{
Calls the function @racket[proc] with the same expansion context and hygiene behavior as calling a function defined by @racket[define/hygienic], except that the @racket{library local binding context} is always @tech{sealed}.
}
...
When a DSL's syntax has Racket subexpression positions, the DSL expander needs to call the Racket expander via @racket[local-expand]. The following operations help connect the Racket expansion with the library-managed binding context.
...
DSL syntax may bind variables that should be accessible from Racket syntax. Use the @racket[racket-var] datatype to represent Racket variable bindings in the @tech{library local binding context} via @racket[bind!].
}


@subsection{Disappeared uses and bindings}

For DrRacket to provide behaviors such as binding arrows, it needs to know which identifiers act as bindings and references. In the case of DSL code these identifiers may not be present as bindings and references in the program's compilation to Racket. The @racket[bind!] and @racket[lookup] operations automatically record such @deftech{disappeared uses} and @deftech{disappeared bindings}.

DrRacket looks for information about disappeared uses and bindings on syntax in fully-expanded modules (see @secref["Syntax_Properties_that_Check_Syntax_Looks_For" #:doc '(lib "scribblings/tools/tools.scrbl")]). This library inserts extra syntax without runtime meaning into the expanded module to carry this information.
